...
application {
    ...
    // named after the Gradle project ("daemon"), which says nothing on an operator's disk or in a
    // container image. `base.archivesName` in java-conventions renames only the jar.
    applicationName = "scalardb-saga-daemon"
...
val imageName = "ghcr.io/scalar-labs/scalardb-saga-daemon"
...
val dockerContext by tasks.registering(Sync::class) {
    description = "Assembles the Docker build context for the daemon image."
    group = "distribution"
...
// Local convenience only: single-architecture, loaded into the local daemon. The release workflow
// builds the same context for both architectures with attestations, so this task deliberately does
// not push.
tasks.register<Exec>("dockerBuild") {
    description = "Builds the daemon image for the local architecture from the assembled context."
    group = "distribution"
