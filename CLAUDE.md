...
- **Auto-PR** (`.github/workflows/auto-pr.yaml`) — on merge into `main` or a version branch, opens cherry-pick backport PRs on every line named by a "ScalarDB Saga <version>" GitHub project attached to the merged PR (shared `scalar-labs/actions` reusable workflow; conflicting cherry-picks become draft PRs with instructions)
