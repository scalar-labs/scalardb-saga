...
/**
 * Thrown by an {@link HttpRequestSpec#send() HTTP call} when it fails. Carries a {@code retryable}
 * flag derived from the response status (or the {@code X-Saga-Retryable} override). {@code
 * HttpServiceInvoker} maps it to a {@code StepExecutionException} (forward) or {@code
 ...
 * a lambda can catch it and inspect the error body/status. For transport, policy, encode, or decode
 * failures there is no response and {@link #response()} is empty.
 *
 * <p>Lambda authors may also throw this directly to signal a business failure with explicit
 * retryability, e.g. {@code throw new HttpCallException("insufficient funds", false)}.
 */
