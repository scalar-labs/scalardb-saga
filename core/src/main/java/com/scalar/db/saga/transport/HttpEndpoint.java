...
public final class HttpEndpoint implements AutoCloseable {
  ...
  /**
   * Builds an endpoint from {@code config}, creating a {@link HttpClient.Redirect#NEVER} client if
   * none was supplied.
   */
  public static HttpEndpoint create(HttpServiceConfig config) {
    ...
    if (supplied != null) {
      ...
      // JDK would follow without re-checking the allowlist (SSRF-via-redirect).
